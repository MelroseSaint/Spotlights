...
import { Upload, X, Music, Video, MapPin, Tag, AlertCircle, CheckCircle2 } from "lucide-react";
...
export function UploadForm({ userId, onSuccess, onCancel }: UploadFormProps) {
  ...
  const validateForm = () => {
    ...
    if (!mediaFile) {
      newErrors.media = "Please upload a media file (audio or video)";
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (mediaFile) {
        ...
        const storageId = await (window as any).convex.upload({
          file: mediaFile,
          storageId: "media",
        });
        mediaUrl = storageId;
      ...
      if (thumbnailFile) {
        const thumbStorageId = await (window as any).convex.upload({
          file: thumbnailFile,
          storageId: "thumbnails",
        });
        thumbnailUrl = thumbStorageId;
      }
      ...
      <div className="p-6 border-b border-white/10">
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-zinc-500 mt-1">
              {remaining} of {uploadStatus?.maxContentAllowed ?? 10} uploads remaining
            </p>
          </div>
          {onCancel && (
            <button 
        ...
        {!canUpload && (
          <div className="mt-4 p-4 bg-red-500/10 border border-red-500/20 rounded-xl flex items-center gap-3 text-red-400">
            <AlertCircle size={20} />
            <span>Upload limit reached. Delete content or upgrade your tier to upload more.</span>
          </div>
        )}
      </div>
      ...
      <form onSubmit={handleSubmit} className="p-6 space-y-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div>
              ...
              className={`w-full py-8 border-2 border-dashed rounded-xl transition-all flex flex-col items-center justify-center gap-2 ${
                errors.media
                  ...
                  : mediaFile
                    ...
                    : "border-white/10 bg-zinc-800/50 hover:border-amber-500/50"
              }`}
            >
              {mediaFile ? (
                <>
            ...
            {errors.media && (
              <p className="text-red-400 text-sm mt-1">{errors.media}</p>
            )}
          </div>
          ...
          <div>
            ...
            >
              {thumbnailFile ? (
                <>
                  <CheckCircle2 size={32} className="text-green-500" />
                  <span className="text-green-400 font-bold">{thumbnailFile.name}</span>
                </>
              ) : (
                <>
                  <Video size={32} className="text-zinc-500" />
                  <span className="text-zinc-400">Upload thumbnail</span>
                </>
              )}
            </button>
          </div>
        ...
        <div>
          ...
          <div className="flex gap-2 mb-2">
            ...
            <button
              ...
              className="px-4 py-2 bg-zinc-800 border border-white/10 rounded-xl text-white hover:bg-zinc-700 transition-colors"
            >
              <Tag size={18} />
            </button>
          </div>
        ...
        <div className="flex gap-4">
          {onCancel && (
            <button
              type="button"
              onClick={onCancel}
              className="flex-1 py-4 rounded-xl border border-white/10 text-zinc-400 font-bold hover:bg-white/5 transition-all"
            >
              Cancel
            </button>
          )}
        ...
        <p className="text-center text-zinc-600 text-xs">
          By uploading, you confirm this is your original content and it meets our community guidelines.
        </p>
      </form>
    </div>
  );
